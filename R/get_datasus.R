...
preprocess_SIA <- function(raw_SIA,
  ...
  outputSIA <- raw_SIA %>%
    ...
    mutate(DT_CMP = ym(PA_CMP),
           ...
           NM_MES_CMP = str_to_title(month(DT_CMP, label=TRUE, abbr = TRUE)),
           NM_ANOMES_CMP = str_glue("{NM_MES_CMP} de {ANO_CMP}"),
           DT_MVM = ym(PA_MVM),
           ANO_MVM = year(DT_MVM),
    ...
    select(`Mês/Ano de Atendimento` = DT_CMP,
           ...
           `Mês de Atendimento` = MES_CMP,
           `Ano/Mês de Atendimento` = PA_CMP,
           `Mês/Ano de Atendimento [2]` = NM_ANOMES_CMP,
           `Mês/Ano de Processamento` = DT_MVM,
           `Ano de Processamento` = ANO_MVM,
           `Mês de Processamento` = MES_MVM,
           ...
           `Valor Apresentado` = PA_VALAPR,
           `Valor Aprovado` = PA_VALPRO,
